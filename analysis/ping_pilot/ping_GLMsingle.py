...
def con_dm(ev_df, design_opt):
    ...
    dm = np.zeros((len(np.arange(0, 
                                 design_opt['total_time'], 
                                 design_opt['pseudo_TR'])), 
                   len(design_opt['angles'])), dtype=int)
    for index, row in onset_df.iterrows():
        TR_ind = int((row['onset'] - design_opt['fixation_dur']*10)/(design_opt['pseudo_TR']*10))
...
def fit_GLMsingle(design_opt, path_opt, GLMsingle_opt, output_typeC_retinamap=False, output_typeD_retinamap=False):
